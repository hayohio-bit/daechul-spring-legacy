</div> <!-- end container -->
<footer
    style="text-align: center; padding: 2rem; color: var(--text-muted); border-top: 1px solid var(--border-color); margin-top: 4rem; font-size: 0.8rem;">
    <div style="margin-bottom: 0.5rem;">
        <a href="https://github.com/hayohio-bit" target="_blank"
            style="color: var(--text-muted); text-decoration: none; margin: 0 8px; font-size: 0.8rem;">GitHub</a>
        <a href="mailto:hayohio@gmail.com"
            style="color: var(--text-muted); text-decoration: none; margin: 0 8px; font-size: 0.8rem;">Contact</a>
    </div>
    <p style="margin: 0.2rem 0;">&copy; 2026 <b>Sun Ha-young</b>. All rights reserved.
    </p>
    <p style="font-size: 0.7rem; opacity: 0.7; margin-top: 0.3rem;">Legacy Migration Project based on Spring Framework 6
    </p>
</footer>
</body>

</html>